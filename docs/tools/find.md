...
## Flow
1. **Lexical scan.** `listFiles()` walks the root (gitignore-aware, no hidden files, no symlinks) and drops build output, lockfiles, binary extensions, and credential files; `grepIndex()` runs one native grep for all keywords and counts per-keyword occurrences on matching lines. Both run concurrently. `idf()` weights rare keywords higher (clamped to `[0.5, 6]`); `fileScore()` ranks every file by weighted log-frequency plus a bonus for keywords in the path.
