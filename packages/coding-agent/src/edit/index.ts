...
export class EditTool implements AgentTool<TInput> {
	...
	#policy(rawInput: boolean, context: ResolveContext): EditPolicy {
		...
		const planWritableRoots: string[] = [];
		for (const [scheme, spec] of router.specs()) {
			urlSchemes.push(scheme);
			if (spec.write?.scope !== "sandbox") continue;
			const root = router.locateSync(`${scheme}://`, context);
			if (root !== undefined) planWritableRoots.push(root);
		}
		return {
	...
	async #resolveUrl(rawUrl: string, context: ResolveContext): Promise<EditUrlResolution> {
		...
		if (!spec.write) return { error: `${url} is read-only`, planWritable: false };
		try {
